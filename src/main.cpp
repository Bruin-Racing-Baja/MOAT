...
void setup()
{
  ...
  Log.notice("status, rpm, act_vel, enc_pos, in_trig, out_trig, s_time, f_time, o_vol, o_curr, couple, wheel_speed, wheel_count, estop" CR);
  save_log();
...
void loop()
{
  ...
  else
  {
    ...
    Log.notice("%d, %d, %d, %d, %d, %d, %d, %d, %F, %F, %d, %d, %d, %d" CR, o_control[0], o_control[1], o_control[2], o_control[7],
               o_control[3], o_control[4], o_control[5], o_control[6], o_control[8], (o_control[9] * 1000.0),
               cooler_o.get_temperature(), o_control[11], o_control[12], estop_pressed);
    estop_pressed = 0;
  }
